...
async def benchmark_stealth_profile(
    ...
    try:
        from linkedin_mcp_server.scraper.stealth.controller import (
            StealthController, PageType, ContentTarget
        )
...
def print_benchmark_results(
    ...
    for result in sorted(results, key=lambda x: x.get("avg_duration", float('inf'))):
        ...
        print(f"   Range: {result['min_duration']:.1f}s - {result['max_duration']:.1f}s")
    ...
    if improvements:
        ...
        for profile_name, improvement in sorted(improvements.items(), key=lambda x: x[1]["improvement_pct"], reverse=True):
            duration = improvement["avg_duration"]
            improvement_pct = improvement["improvement_pct"]
            
            if improvement_pct > 0:
                print(f"✅ {profile_name}: {improvement_pct:.1f}% faster ({duration:.1f}s avg)")
            elif improvement_pct == 0:
                print(f"📌 {profile_name}: {duration:.1f}s avg (baseline)")
            else:
                print(f"⚠️  {profile_name}: {abs(improvement_pct):.1f}% slower ({duration:.1f}s avg)")
    ...
    for profile_name, target in targets.items():
        ...
        if duration <= target:
            ...
            print(f"❌ {profile_name}: {duration:.1f}s (target: <{target}s) - MISSED TARGET")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark LinkedIn MCP stealth system performance")
    parser.add_argument(
        "--profiles",
    ...
    logger.info(f"Starting benchmark: {len(profiles)} profiles x {len(urls)} URLs x {args.iterations} iterations")
