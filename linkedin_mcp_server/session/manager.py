...
class PlaywrightSessionManager:
    ...
    @classmethod
    async def get_or_create_session(cls, authentication: str, headless: bool = True) -> LinkedInSession:
        """
        ...
        async with cls._session_locks[session_id]:
            ...
            if session_id in cls._sessions:
                ...
                try:
                    ...
                    if hasattr(session, 'is_authenticated') and await session.is_authenticated():
                        logger.info("Using existing authenticated LinkedIn session")
                        return session
                    else:
                        logger.info("Existing session not authenticated, creating new one")
