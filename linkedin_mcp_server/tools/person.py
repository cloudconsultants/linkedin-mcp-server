...
async def get_person_profile_minimal(linkedin_username: str) -> Dict[str, Any]:
    ...
    try:
        ...
        stealth_system = (
            "NEW (centralized)" if stealth_config["use_new_stealth"] else "LEGACY"
        )
        logger.info(
            f"Using {stealth_system} stealth system with {stealth_config['stealth_profile']} profile"
        )
        ...
        # Log performance
        logger.info(
            f"Profile scraping completed in {duration:.1f}s using {stealth_system} system"
        )
...
async def get_person_profile(linkedin_username: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Log performance
        logger.info(
            f"Comprehensive profile scraping completed in {duration:.1f}s using {stealth_system} system"
        )
