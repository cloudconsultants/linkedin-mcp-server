...
class StealthBrowserContextManager:
    ...
    async def navigate_to_profile_safely(self, page: Page, linkedin_url: str) -> bool:
        ...
        try:
            ...
            from .behavioral import navigate_to_profile_stealthily, detect_linkedin_challenge
            
            # Navigate using stealth patterns
            await navigate_to_profile_stealthily(page, linkedin_url, self.stealth_config)
            
            # Check for detection after navigation
            if await detect_linkedin_challenge(page):
                raise LinkedInDetectionError("Detection suspected after profile navigation")
