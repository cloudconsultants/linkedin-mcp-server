...
class StealthManager:
    ...
    async def _create_botright_context(
        self, storage_state_path: Optional[str] = None
    ) -> BrowserContext:
        ...
        try:
            ...
            logger.error(
                f"Botright not available (dependency conflict resolved by using Patchright only): {e}"
            )
    ...
    async def enforce_rate_limit(self):
        ...
        if time_since_last < 60:  # Less than 60 seconds since last profile
            ...
            if self.config.stealth_wait_message:
                logger.info(
                    f"Rate limiting: waiting {wait_time:.1f}s before next profile"
                )
