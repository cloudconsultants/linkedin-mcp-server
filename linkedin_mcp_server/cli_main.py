...
def clear_keychain_and_exit() -> None:
    ...
    print(f"\n🔑 Clear LinkedIn data from {keyring_name}?")
...
async def initialize_session_with_auth(authentication: str) -> None:
    """
    Phase 2: Initialize Playwright session using existing authentication.

    Args:
        authentication: LinkedIn session cookie

    Raises:
        Various exceptions if session creation or login fails
    """
    config = get_config()
    ...
    logger.info("Initializing Playwright session and logging in...")

    try:
        # Create session and login with provided authentication
        await PlaywrightSessionManager.get_or_create_session(
            authentication, headless=config.chrome.headless
        )
        logger.info("✅ Playwright session initialized and authenticated successfully")
