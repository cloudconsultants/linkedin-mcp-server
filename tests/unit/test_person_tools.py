...
class TestPersonTools:
    ...
    async def test_minimal_profile_success(self, mock_person_minimal):
        ...
        with patch('linkedin_mcp_server.error_handler.safe_get_session') as mock_get_session:
    ...
    async def test_full_profile_success(self, mock_person_full):
        ...
        with patch('linkedin_mcp_server.error_handler.safe_get_session') as mock_get_session:
            ...
            assert edu["institution"] == "University of California"  # Mapped from institution_name
            ...
            honor_titles = [acc["title"] for acc in accomplishments if acc["category"] == "Honor"]
            language_titles = [acc["title"] for acc in accomplishments if acc["category"] == "Language"]
